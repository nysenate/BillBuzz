 ...
 * @since DbUtils 1.1
 */
public class PrefixedBeanProcessor {
     ...
     * @since 1.5
     */
    public PrefixedBeanProcessor(Map<String, String> columnToPropertyOverrides) {
        super();
     ...
     */
    public <T> T toBean(ResultSet rs, Class<T> type, String prefix) throws SQLException {

        PropertyDescriptor[] props = this.propertyDescriptors(type);

        ResultSetMetaData rsmd = rs.getMetaData();
        int[] columnToProperty = this.mapColumnsToProperties(rsmd, props, prefix);
     ...
     */
    public <T> List<T> toBeanList(ResultSet rs, Class<T> type, String prefix) throws SQLException {
     ...
     */
    protected PropertyDescriptor[] propertyDescriptors(Class<?> c)
        throws SQLException {
     ...
     */
    protected int[] mapColumnsToProperties(ResultSetMetaData rsmd, PropertyDescriptor[] props, String prefix) throws SQLException {
        ...
        for (int col = 1; col <= cols; col++) {
            ...
            // Try to map columns to properties using propertyName
            ...
            // If that fails, try it again with the prefix
            if (columnToProperty[col] == PROPERTY_NOT_FOUND) {
                for (int i = 0; i < props.length; i++) {
                    if (propertyName.equalsIgnoreCase(prefix+props[i].getName())) {
                        columnToProperty[col] = i;
                        break;
                    }
                }
