...
public class SendDigests extends BaseScript
{
  ...
  protected Options getOptions()
  {
    ...
    options.addOption("n", "dryrun", false, "Prevents the digests from actually being mailed.");
    options.addOption("s", "startat", true, "Starts the digest mailing from the indicated user id.");
    return options;
  }
    ...
    for (BillBuzzUser user : getUsers(runner, startAt)) {
      ...
      for (BillBuzzSubscription subscription : user.getSubscriptions()) {
        if (subscription.getCategory().equals("all")) {
          ...
          BillBuzzSenator senator = senatorsByShortName.get(subscription.getValue().toLowerCase());
          if (senator != null) {
            userSubscriptions.add(senator);
          }
          else {
            logger.error("\tBad subscription to sponsor: "+subscription.getValue());
          }
        }
        else {
